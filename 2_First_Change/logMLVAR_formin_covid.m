function [logML,betahat,sigmahat]=logMLVAR_formin_covid(par,y,x,lags,T,n,b,...
    MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,hyperpriors,priorcoef,Tcovid)
...
if isempty(Tcovid)
    ...
    %end
    invweights(Tcovid+2)=eta(3);
    invweights(Tcovid+3:T)=eta(4); 
...
bbb=diag(1./sqrt(psi))*(epshat'*epshat + ...
    (betahat-b)'*diag(1./omega)*(betahat-b))*diag(1./sqrt(psi));
...
if sur==1 || noc==1
    ...
    bbb=diag(1./sqrt(psi))*(epshatd'*epshatd + ...
        (betahatd-b)'*diag(1./omega)*(betahatd-b))*diag(1./sqrt(psi));
